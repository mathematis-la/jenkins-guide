      ...
      parallel {
        ...
        stage('Quality Tests') {
          steps {
            ...
            sh 'docker tag nodeapp-dev:trunk mathematis/nodeapp-dev:latest'
            sh 'docker push mathematis/nodeapp-dev:latest'
          }
        }
      }
      post {
            ...
            steps {
                    retry(3) {
                        timeout(time:10, unit: 'MINUTES') {
                            sh 'docker tag nodeapp-dev:trunk mathematis/nodeapp-prod:latest'
                            sh 'docker push mathematis/nodeapp-prod:latest'
                            sh 'docker save mathematis/nodeapp-prod:latest | gzip > nodeapp-prod-golden.tar.gz'
                        }
                    }
